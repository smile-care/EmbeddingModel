...
def compute_crop_region(
    ...
    # 确保crop_size余量在四边分别为max_box_size的20%
    min_required_crop_size = int(max_box_size * (1 + 2 * expand_ratio))
    
    # 根据max_box_size大小选择crop策略，确保满足余量要求
    if max_box_size < crop_sizes[0]:
        crop_size = max(crop_sizes[0], min_required_crop_size)
        use_expand = False
    elif max_box_size < crop_sizes[1]:
        crop_size = max(crop_sizes[1], min_required_crop_size)
        use_expand = False
    elif max_box_size < crop_sizes[2]:
        crop_size = max(crop_sizes[2], min_required_crop_size)
        use_expand = False
    else:
        crop_size = min_required_crop_size
        use_expand = True
    
    if use_expand:
        expand_w = int(max_box_size * expand_ratio)
        expand_h = int(max_box_size * expand_ratio)
...
def extract_patches_from_image_json(
    ...
    """
    从图像和JSON标注文件提取所有instance的patches
    ...
    for idx, instance_mask in enumerate(instance_masks):
        ...
        crop_x_min, crop_y_min, crop_x_max, crop_y_max = compute_crop_region(
            bbox, (img_h, img_w), max_box_size, crop_sizes
        )
...
def process_folder(
    ...
    max_box_size_dict = defaultdict(int)
    for json_path in tqdm(json_files, desc="计算max_box_size"):
        # 查找对应的图像文件
        ...
        for label_data in labels_data:
            ...
            polygon_mask = polygon_to_mask(points, image_h, image_w)
            bbox = extract_bbox_from_mask(polygon_mask)
            if bbox is None:
                continue
            x_min, y_min, x_max, y_max = bbox
            bbox_max = max(x_max - x_min, y_max - y_min)
            max_box_size_dict[label] = max(max_box_size_dict.get(label, 0), bbox_max)

    print(max_box_size_dict)
    ...
    for json_path in tqdm(json_files, desc="处理JSON文件"):
        ...
        try:
            patches_info = extract_patches_from_image_json(
                ...
                label=label,
                max_box_size=max_box_size_dict[label]
            )
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default="data/zhenyu_data/60194_pipeline/60194-CCD1-客户喷码区", help='输出目录')
