...
from .models.losses import SimilarityTargetLoss, SupervisedContrastiveLoss
...
def validate(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validating"):
            ...
            # 计算loss（为了兼容，仍然使用两个view，但第二个view与第一个相同）
            embeddings = torch.cat([embeddings1, embeddings1], dim=0)  # (2B, D)
            labels_duplicated = torch.cat([labels, labels], dim=0)  # (2B,)
...
def main():
    ...
    parser.add_argument('--use_eval', action='store_true', default=False, help='是否进行验证')
